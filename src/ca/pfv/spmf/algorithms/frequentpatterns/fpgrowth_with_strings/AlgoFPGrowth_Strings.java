...
/**
 * This is an implementation of the FPGROWTH algorithm (Han et al., 2004) that
 * take as input a transaction database where items are represented by strings
 * rather than integers. FPGrowth is described here: <br/>
 * <br/>
 * 
 * Han, J., Pei, J., & Yin, Y. (2000, May). Mining frequent patterns without
 * candidate generation. In ACM SIGMOD Record (Vol. 29, No. 2, pp. 1-12). ACM
 * <br/>
 * <br/>
 * 
...
public class AlgoFPGrowth_Strings {
	 ...
	 * 
	 * @param input
	 *            the file path of an input transaction database.
	 * @param output
	 *            the path of the desired output file
	 * @param minsupp
	 *            minimum support threshold as a percentage (double)
	 * @throws IOException
	...
	public void runAlgorithm(String input, String output, double minsupp) throws FileNotFoundException, IOException {
		...
		while (((line = reader.readLine()) != null)) {
			...
			for (String itemString : lineSplited) {
				...
				// otherwise not because it cannot be part of a frequent
				// itemset.
	...
	// System.out.println(indentation + "NODE : " + node.itemID + " COUNTER" +
	// node.counter);
	 ...
	 *             exception if error while writing the file
	 */
	private void scanDatabaseToDetermineFrequencyOfSingleItems(String input, final Map<String, Integer> mapSupport)
	 ...
	 */
	private void fpgrowth(FPTree_Strings tree, String[] prefixAlpha, int prefixSupport, Map<String, Integer> mapSupport)
			throws IOException {
	 ...
	 *             exception if error writing the output file
	 */
	private void fpgrowthMoreThanOnePath(FPTree_Strings tree, String[] prefixAlpha, int prefixSupport,
			Map<String, Integer> mapSupport) throws IOException {
